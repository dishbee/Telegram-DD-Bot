...
# ===== STATE (ephemeral) =====
ORDERS = {}        # order_key -> {last2, vendors:{vendor:[items]}, addr/name/phone}
...
# ===== RENDERING =====
def last2(n): s = str(n); return s[-2:] if len(s) >= 2 else s
...
def telegram_updates():
    ...
    if not cb: return "OK", 200

    data = cb.get("data",""); cb_id = cb.get("id")
    m = cb.get("message",{}); chat_id = m.get("chat",{}).get("id"); mid = m.get("message_id")
    parts = data.split(":"); kind = parts[0]
    ...
    if kind=="req_asap":
        ...
        for vendor in od.get("vendors",{}).keys():
            gid = VENDOR_GROUP_MAP.get(vendor); if not gid: continue
        ...
        tg_answer(cb_id, "ASAP sent."); return "OK", 200

    if kind=="req_time":
        _, order_key = parts
        now = datetime.now().astimezone()
        base = now + timedelta(minutes=(10 - (now.minute % 10)) % 10)
        # build inline selector right on MDG message
        rows=[]; row=[]
        for i in range(6):
    ...
    if kind=="req_time_sel":
        ...
        tg_answer(cb_id, f"Requested {hhmm}."); return "OK", 200
    ...
    if kind in ("rest_works","rest_later","rest_will"):
        ...
        if not last_req:
            tg_answer(cb_id, "No time was requested yet.", True); return "OK", 200
        ...
        tg_answer(cb_id, "Noted."); return "OK", 200
...
def shopify_webhook():
    ...
    mdg_lines = [f"<b>dishbee + {', '.join(vendors.keys())}</b>",
                 f"Order: #{last2num}"]
    street = " ".join([ship.get("address1",""), ship.get("address2","")]).strip()
    if street: mdg_lines.append(f"<b>{street}</b>")
    if ship.get("zip"): mdg_lines.append(f"<b>{ship.get('zip')}</b>")
