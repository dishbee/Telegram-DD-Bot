...
❌ NO ADDING DEFENSIVE CODE WITHOUT READING ACTUAL DATA - Don't add fallbacks/length checks without verifying what the data actually contains. Read where data comes from, trace through code
❌ NO ASSUMING DATA FORMAT FROM COMMENTS - Comments lie. Code is truth. If comment says "take last 2 digits", verify by reading where the value is SET
❌ NO SKIPPING MENTAL TESTING - Before writing code, mentally trace the logic. Test edge cases in your head FIRST
...
ADDING DEFENSIVE CODE WITHOUT UNDERSTANDING DATA FORMAT (Commit 4e02770 - Added if len() >= 2 else fallback when extracting order number without checking what order['name'] actually contains. Result: "dishbee #02"[-2:] worked but fallback returned full string "dishbee #02". Always verify data format by reading source code before adding logic)
